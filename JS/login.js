...
function login() {
    ...
    for (let i = 0; i < userList.length; i++) {
        if (user == userList[i].username && pass == userList[i].password) {
            ...
            localStorage.setItem("image", JSON.stringify(userList[i].image));
            window.location.href = "Profile_Page.html";
            return true;
        }
    }
...
function details() {
    if (localStorage.getItem('name')) {
        ...
        if (img) {
            document.querySelector("#imgProfile").setAttribute("src", img);
        }
    }
    else {
...
function img() {
    document.getElementById("imgP").addEventListener("change", function () {
...
function loadData() {
    ...
    document.getElementById("imgD").setAttribute("src", JSON.parse(localStorage.getItem("image")));
